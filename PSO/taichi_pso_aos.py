...
def main(n_dim, low_bound, high_bound, vmin, vmax, wmin, wmax, c1, c2):
    ...
    ti.root.dense(ti.i, POP_SIZE).place(Envo_V)

    Envo_X = ti.Vector.field(N_DIM, dtype=ti.f32)

    ti.root.dense(ti.i, POP_SIZE).place(Envo_X)

    Envo_Y = ti.field(dtype=ti.f32)

    ti.root.dense(ti.i, POP_SIZE).place(Envo_Y)

    Envo_P_best_X = ti.Vector.field(N_DIM, dtype=ti.f32)

    ti.root.dense(ti.i, POP_SIZE).place(Envo_P_best_X)

    Envo_P_best_Y = ti.field(dtype=ti.f32)

    ti.root.dense(ti.i, POP_SIZE).place(Envo_P_best_Y)

    Envo_G_best_X = ti.Vector.field(N_DIM, dtype=ti.f32)

    ti.root.dense(ti.i, 1).place(Envo_G_best_X)
