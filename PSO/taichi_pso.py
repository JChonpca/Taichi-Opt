# -*- coding: utf-8 -*-
"""
Created on Sat Apr  1 08:34:52 2023
...
def update(g:int):
    ...
    for i,j in ti.ndrange(POP_SIZE,N_DIM):
        ...
        Envo_V[i][j] = W*Envo_V[i][j] + C1*r1*(Envo_P_best_X[i][j] - Envo_X[i][j]) + C2*r2*(Envo_G_best_X[0][j] - Envo_X[i][j])
...
def main(n_dim, low_bound, high_bound, vmin , vmax, wmin, wmax, c1, c2):
    ...
    Envo_V = ti.Vector.field(N_DIM, dtype=ti.f32,shape=(POP_SIZE))
    
    Envo_X = ti.Vector.field(N_DIM, dtype=ti.f32, shape=(POP_SIZE))
    
    Envo_Y = ti.field(dtype=ti.f32, shape=(POP_SIZE))

    Envo_P_best_X = ti.Vector.field(N_DIM, dtype=ti.f32, shape=(POP_SIZE))
    
    Envo_P_best_Y = ti.field(dtype=ti.f32, shape=(POP_SIZE))
    
    Envo_G_best_X = ti.Vector.field(N_DIM, dtype=ti.f32, shape=(1))
    
    Envo_G_best_Y = ti.field(dtype=ti.f32, shape=(1))
