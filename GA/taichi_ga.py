...
def init_pop(g: int):
    ...
    for i,j in ti.ndrange(POP_SIZE[None],DNA_SIZE[None]*N_DIM[None]):
...
def select(g: int):
    ...
    for i in range(POP_SIZE[None]):
        
        best_index = -1
        
        best_y = -1.0
        
        for j in range(TOURN_SIZE[None]):
            
            if j == 0:
                
                best_index = int(ti.random()*POP_SIZE[None])
                
                best_y = Envo_Y[g-1,best_index][0]
            
            else:
                
                tmp_best_index = int(ti.random()*POP_SIZE[None])
...
def exchange(n1: int, 
    ...
    for i in range(DNA_SIZE[None]*N_DIM[None]):
...
def crossover(g: int):
    ...
    for i in range(int(POP_SIZE[None]/2)):
        
        if ti.random() < CROSSOVER_RATE[None]:
            ...
            n1 = int(n1*DNA_SIZE[None]*N_DIM[None])
            
            n2 = int(n2*DNA_SIZE[None]*N_DIM[None])
...
def mutation(g: int):
    ...
    for i,j in ti.ndrange(POP_SIZE[None],DNA_SIZE[None]*N_DIM[None]):
        ...
        if ti.random() < MUTATION_RATE[None]:
...
#         index = int(ti.random()*POP_SIZE[None])
...
def extractx(g: int):
    ...
    for i,j in ti.ndrange(POP_SIZE[None],DNA_SIZE[None]*N_DIM[None]):
        ...
        dim_count = int(j // N_DIM[None])
        
        dim_index = int(j % N_DIM[None])
...
def transx(g: int,
    ...
    for i in range(DNA_SIZE[None]):
        
        num += (Envo_EDX[g,n,m][i])*(2**(DNA_SIZE[None]-i))
    
    num = num/float(2**DNA_SIZE[None]-1)
...
def decodex(g: int):
    ...
    for i,j in ti.ndrange(POP_SIZE[None],N_DIM[None]):
...
def GA_RUN():
    ...
    for i in range(1,N_GENERATIONS[None]):
...
def main(n_dim, low_bound, high_bound):
    ...
    N_GENERATIONS[None] = 1000
    
    TOURN_SIZE[None] = 5

    CROSSOVER_RATE[None] = 0.8

    MUTATION_RATE[None] = 0.05
    
    
    HIGH_BOUND = ti.field(ti.f32, shape=N_DIM[None])
    ...
    LOW_BOUND  = ti.field(ti.f32, shape=N_DIM[None])
    ...
    Envo_EX  = ti.Vector.field(n= DNA_SIZE[None]*N_DIM[None], dtype=ti.f32, shape=(N_GENERATIONS[None], POP_SIZE[None]))
    
    Envo_SEX = ti.Vector.field(n= DNA_SIZE[None]*N_DIM[None], dtype=ti.i32, shape=(N_GENERATIONS[None], POP_SIZE[None]))
    
    Envo_CEX = ti.Vector.field(n= DNA_SIZE[None]*N_DIM[None], dtype=ti.i32, shape=(N_GENERATIONS[None], POP_SIZE[None]))
    
    Envo_MEX = ti.Vector.field(n= DNA_SIZE[None]*N_DIM[None], dtype=ti.i32, shape=(N_GENERATIONS[None], POP_SIZE[None]))
        
    Envo_EDX  = ti.Vector.field(n= DNA_SIZE[None], dtype=ti.f32, shape=(N_GENERATIONS[None],POP_SIZE[None],N_DIM[None]))

    Envo_DX  = ti.Vector.field(n= N_DIM[None], dtype=ti.f32, shape=(N_GENERATIONS[None],POP_SIZE[None]))

    Envo_Y   = ti.Vector.field(n= 1, dtype=ti.f32, shape=(N_GENERATIONS[None],POP_SIZE[None]))
    
    Envo_Best_Y = ti.Vector.field(n= 1, dtype=ti.f32, shape=N_GENERATIONS[None])
    
    Envo_Best_Index = ti.Vector.field(n= 1, dtype=ti.f32, shape=N_GENERATIONS[None])
