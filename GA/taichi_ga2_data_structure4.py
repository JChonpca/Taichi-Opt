...
ti.init(arch=ti.gpu, default_fp=ti.f32 ,debug=True, kernel_profiler=True)
...
def select(g: int):
    ...
    # best_y : 随机访问 麻了
    # Envo_SEX : 写入 SOA 更好
    # Envo_SEX 和 Envo_EX 怎么place都不影响 (1-2n) 因为 best_index 是随机的
...
def exchange(n1: int, 
    ...
    # Envo_SEX : AOS 结构更好 ? 冲突！ 取AOS 循环数更大
    
    # Envo_CEX 和 Envo_SEX 要一起place, 读取距离会降低（点对点）
    
    for i in range(DNA_SIZE*N_DIM):
        
        if 0 <= i < n1:
            
            Envo_CEX[2*n,i] = Envo_SEX[2*n,i]
            
            Envo_CEX[2*n+1,i] = Envo_SEX[2*n+1,i]
                
        elif n1 <= i < n2:
            
            Envo_CEX[2*n,i] = Envo_SEX[2*n+1,i]   
            
            Envo_CEX[2*n+1,i] = Envo_SEX[2*n,i]        
...
def mutation(g: int):
    ...
    # Envo_MEX 和 Envo_CEX 要一起place, 读取距离会降低（点对点）
    
    # Envo_MEX: SOA 更好 但是处罚几率很低
    
    for i,j in ti.ndrange(POP_SIZE,DNA_SIZE*N_DIM):
        
        Envo_MEX[i,j] = Envo_CEX[i,j]
        
        if ti.random() < MUTATION_RATE:
            
            Envo_MEX[i,j] = int(ti.random()*2)
...
def extractx(g: int):
    ...
    for i,j in ti.ndrange(POP_SIZE,DNA_SIZE*N_DIM):
        ...
        dim_count = int(j // DNA_SIZE) # 整数
        
        dim_index = int(j % DNA_SIZE) # 余数
        
        Envo_EDX[i,dim_count][dim_index] = Envo_EX[i,j]
...
def main(n_dim, low_bound, high_bound):
    ...
    # Envo_CEX = ti.field(dtype=ti.i32, shape=(POP_SIZE, DNA_SIZE*N_DIM))
    
    # Envo_MEX = ti.field(dtype=ti.i32, shape=(POP_SIZE, DNA_SIZE*N_DIM))
    
    Envo_EX  = ti.field(dtype=ti.i32)
    
    Envo_SEX = ti.field(dtype=ti.i32)
    
    Envo_CEX = ti.field(dtype=ti.i32)
    
    Envo_MEX = ti.field(dtype=ti.i32)
    ...
    ti.root.dense(ti.ij, (POP_SIZE, DNA_SIZE*N_DIM)).place(Envo_CEX)
    
    ti.root.dense(ti.ij, (POP_SIZE, DNA_SIZE*N_DIM)).place(Envo_MEX)
    
    
    # ti.root.dense(ti.jk, (POP_SIZE, DNA_SIZE*N_DIM)).place(Envo_SEX, Envo_CEX, Envo_MEX, Envo_EX)
