...
def decodex3(g: int):
    ...
    for i,j in ti.ndrange(POP_SIZE,DNA_SIZE*N_DIM):
        ...
        # old_j DNA_SIZE*N_DIM

        dim_count = int(j // N_DIM) # 整数 # N_DIM new_j m
        
        dim_index = int(j % N_DIM) # 余数  # DNA_SIZE
        
        # Envo_EDX[i,dim_index][dim_count] = Envo_SEX[i,j]
        
        Envo_DX[i][dim_index] += Envo_SEX[i,j]*(2**(DNA_SIZE-dim_count))/float(2**DNA_SIZE-1)*(HIGH_BOUND[dim_index]-LOW_BOUND[dim_index]) + LOW_BOUND[dim_index]/DNA_SIZE
        
        # Envo_DX[i][dim_count] += Envo_SEX[i,j]*(2**(DNA_SIZE-dim_index))/float(2**DNA_SIZE-1)*(HIGH_BOUND[dim_count]-LOW_BOUND[dim_count]) + LOW_BOUND[dim_count]/DNA_SIZE
...
def decodex4(g: int):
    ...
    for i,j in ti.ndrange(POP_SIZE,DNA_SIZE*N_DIM):
        ...
        Envo_DX[i][dim_index] += Envo_EX[i,j]*(2**(DNA_SIZE-dim_count))/float(2**DNA_SIZE-1)*(HIGH_BOUND[dim_index]-LOW_BOUND[dim_index]) + LOW_BOUND[dim_index]/DNA_SIZE

        
        # Envo_DX[i][dim_count] += Envo_EX[i,j]*(2**(DNA_SIZE-dim_index))/float(2**DNA_SIZE-1)*(HIGH_BOUND[dim_count]-LOW_BOUND[dim_count]) + LOW_BOUND[dim_count]/DNA_SIZE
